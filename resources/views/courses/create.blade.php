...
<form action="{{ route('courses.store') }}" method="POST">
  ...
  <div class="form-group">
    ...
    <select class="form-control @if($errors->has('periodo')) is-invalid @endif" name="periodo" id="periodo">
      ...
      <option value="noturno" @if(old('periodo') === 'noturno') selected @endif>noturno</option>
      <option value="matutino" @if(old('periodo') === 'matutino') selected @endif>matutino</option>
      <option value="vespertino" @if(old('periodo') === 'vespertino') selected @endif>vespertino</option>
    </select>
  ...
  <div class="form-group">
    ...
    <input name="duracao" value="{{ old('duracao') }}" type="number" class="form-control @if($errors->has('duracao')) is-invalid @endif" id="duracao" aria-describedby="duracao" placeholder="00">    
